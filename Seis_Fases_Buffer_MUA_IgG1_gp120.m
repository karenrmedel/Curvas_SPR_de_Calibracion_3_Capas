...
% 6: Monocapa 2 (IgG1 b12)
n_pr2=1.542; %Índice de refracción de IgG1
k_pr2=0;
d1_pr2_1=136.75; %(136.75 Mas probable)
d_pr2_1=d1_pr2_1*cos(pi/6);

% 7: Monocapa 3 (gp120)
n_pr3=1.4555; %Índice de refracción de gp120
k_pr3=0;
d_pr3=19.30; %Tamaño de gp120 (19.30 A)

% 8: Buffer HEPES (medio)
n_med=1.3341;
k_med=0;
...
% *************************************************************************
% CÁLCULOS
theta_deg=60:.01:80; % Rango: 50° a 80°. Intervalos: 0.01°
...
% CÁLCULOS
theta_deg=60:.01:80; % Rango: 50° a 80°. Intervalos: 0.01°
theta_ext_A=theta_deg/180*pi;
theta0_A=(pi/4)+asin(1/en_A(1)*sin(theta_ext_A-(pi/4)));

for jtheta=1:length(theta0_A);
    theta_A=theta0_A(jtheta);
    q1_A=sqrt(e_A(1)-en_A(1)^2*sin(theta_A)^2)/e_A(1);
    qn_A=sqrt(e_A(end)-en_A(1)^2*sin(theta_A)^2)/e_A(end);
    
    for j=2:(length(e_A)-1)
        beta_A=d_1_A(j)*2*pi/lambda*sqrt(e_A(j)-en_A(1)^2*sin(theta_A)^2);
        q_A=sqrt(e_A(j)-en_A(1)^2*sin(theta_A)^2)/e_A(j);
        em_A(j,1,1)=cos(beta_A);
        em_A(j,1,2)=-i*sin(beta_A)/q_A;
        em_A(j,2,1)=-i*sin(beta_A)*q_A;
        em_A(j,2,2)=cos(beta_A);
    end
    ...
    for j=2:(length(e_A)-1)
        emtot1_A(:,:)=em_A(j,:,:);
        emtot_A=emtot_A*emtot1_A;
    end
    
    rp_A=((emtot_A(1,1)+emtot_A(1,2)*qn_A)*q1_A-(emtot_A(2,1)+emtot_A(2,2)*qn_A))/...
        ((emtot_A(1,1)+emtot_A(1,2)*qn_A)*q1_A+(emtot_A(2,1)+emtot_A(2,2)*qn_A));
    ref_1_A=rp_A*conj(rp_A);
...
% CÁLCULOS
theta_deg=60:.01:80; % Rango: 50° a 80°. Intervalos: 0.01°
theta_ext_D=theta_deg/180*pi;
theta0_D=(pi/4)+asin(1/en_D(1)*sin(theta_ext_D-(pi/4)));

for jtheta=1:length(theta0_D);
    theta_D=theta0_D(jtheta);
    q1_D=sqrt(e_D(1)-en_D(1)^2*sin(theta_D)^2)/e_D(1);
    qn_D=sqrt(e_D(end)-en_D(1)^2*sin(theta_D)^2)/e_D(end);
    
    for j=2:(length(e_D)-1)
        beta_D=d_1_D(j)*2*pi/lambda*sqrt(e_D(j)-en_D(1)^2*sin(theta_D)^2);
        q_D=sqrt(e_D(j)-en_D(1)^2*sin(theta_D)^2)/e_D(j);
        em_D(j,1,1)=cos(beta_D);
        em_D(j,1,2)=-i*sin(beta_D)/q_D;
        em_D(j,2,1)=-i*sin(beta_D)*q_D;
        em_D(j,2,2)=cos(beta_D);
    end
    ...
    for j=2:(length(e_D)-1)
        emtot1_D(:,:)=em_D(j,:,:);
        emtot_D=emtot_D*emtot1_D;
    end
    
    rp_D=((emtot_D(1,1)+emtot_D(1,2)*qn_D)*q1_D-(emtot_D(2,1)+emtot_D(2,2)*qn_D))/...
        ((emtot_D(1,1)+emtot_D(1,2)*qn_D)*q1_D+(emtot_D(2,1)+emtot_D(2,2)*qn_D));
    ref_1_D=rp_D*conj(rp_D);
...
% VIDRIO/ORO/MUA/IgG1 b12/BUFFER
...
% VIDRIO/ORO/MUA/IgG1 b12/gp120/BUFFER
...
plot(theta_deg,refle_1_C,'-k', 'DisplayName',...
    ['Buffer HEPES, \theta_{min} = ', num2str(thetacr_1_C), '°'])  
hold on;
...
plot(theta_deg,refle_1_B,'-.k', 'DisplayName',...
    ['IgG1 b12,         \theta_{min} = ', num2str(thetacr_1_B), '°']) 
plot(theta_deg,refle_1,'--k', 'DisplayName',...
    ['gp120,              \theta_{min} = ', num2str(thetacr_1), '°']) 
hold off;

title('SIMULACION CURVA SPR MUA+IgG1(136.75 A)+gp120(19.30 A)+HEPES')
ylabel('Reflectancia (%)')
xlabel('Ángulo (°)')