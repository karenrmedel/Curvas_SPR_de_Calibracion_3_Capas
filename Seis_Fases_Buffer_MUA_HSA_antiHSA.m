...
% 6: Monocapa 2 (HSA)
n_pr2=1.575; %Índice de refracción de HSA
k_pr2=0;
d1_pr2_1=55.61; %(55.61 A Mas probable)
d_pr2_1=d1_pr2_1*cos(pi/6);

% 7: Monocapa 3 (antiHSA)
n_pr3=1.542; %Índice de refracción de Anti-HSA
k_pr3=0;
d_pr3=55; %Tamaño de AntiHSA (55 A)
...
% VIDRIO/ORO/MUA/BSA/antiHSA/BUFFER
% Creación de matrices para n, k y d y cálculo de la constante
% dieléctrica de cada capa
...
plot(theta_deg,refle_1_B,'-.k', 'DisplayName',...
    ['HSA,         \theta_{min} = ', num2str(thetacr_1_B), '°']) 
plot(theta_deg,refle_1,'--k', 'DisplayName',...
    ['antiHSA,   \theta_{min} = ', num2str(thetacr_1), '°']) 
hold off;

title('SIMULACION CURVA SPR MUA+HSA(62.74 A)+antiHSA(55 A)+Acetato de Potasio')
ylabel('Reflectancia (%)')
xlabel('Ángulo (°)')